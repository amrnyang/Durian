...
public class QuestionController {
    ...
    @GetMapping("search")
    @CrossOrigin(origins = {"*"}, methods = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT}, maxAge = 3600, allowedHeaders = {}, exposedHeaders = {})
    public SkyResponse searchQuestion(String key, String condition, Integer pageStart, Integer pageRows) {
        if (key == null) {
            return SkyResponse.fail(HttpStatus.INTERNAL_SERVER_ERROR, "未检测到有效的密钥");
        }
        if (!key.equalsIgnoreCase(updateKey)) {
        ...
        List<QuestionDO> questions = questionService.searchQuestion(condition, "ti_keywords", pageStart, pageRows);
        return SkyResponse.success(1).put("questions", questions);
    }
}
